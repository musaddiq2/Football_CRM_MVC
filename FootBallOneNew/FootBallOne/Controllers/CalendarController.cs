...
namespace FootBallOne.Controllers
{
    public class CalendarController : BaseController
    {
        ...
        public CalendarController(ApplicationDbContext context, ILogger<CalendarController> logger, IAcademyContext academyContext)
        : base(academyContext)
        {
            _context = context;
            _logger = logger;
        ...
        public async Task<IActionResult> Index(int? year, int? month, string view = "month")
        {
            ...
            // Fetch events
            var events = await FilterByAcademy(_context.Events.Where(e => e.IsActive))
            ...
            // Fetch camps
            var camps = await FilterByAcademy(_context.Camps.Where(c => c.IsActive))
            ...
            var matches = await FilterByAcademy(_context.MatchInfo.Where(m => m.IsActive && m.MatchDate >= DateTime.UtcNow.Date))
            ...
            // Fetch trainings
            var trainings = await FilterByAcademy(_context.Trainings.Where(t => t.IsActive))
            ...
            var tournaments = await FilterByAcademy(_context.Tournaments.Where(t => t.IsActive && t.EndDate >= DateTime.UtcNow.Date))
            .Select(t => new CalendarItem
            {
                    Id = t.TournamentID,
