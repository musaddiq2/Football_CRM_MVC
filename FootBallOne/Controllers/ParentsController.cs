...
namespace FootBallOne.Controllers
{
    public class ParentsController : BaseController
    {
        ...
        public ParentsController(ApplicationDbContext context, ILogger<ParentsController> logger, IWebHostEnvironment webHostEnvironment, IAcademyContext academyContext)
       : base(academyContext)
        {
            _context = context;
            _logger = logger;
        ...
        public async Task<IActionResult> Index(int? registrationId = null)
        {
            try
            {
                var parents = await FilterByAcademy(_context.Parents)
